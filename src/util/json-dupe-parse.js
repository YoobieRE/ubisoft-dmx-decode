/* eslint-disable */
/**
 * Credit mostly to https://github.com/LuisMayo/json-duplicate-key-handle and https://github.com/nmccready/json-duplicate-key-handle
 * This version supports more than 1 duplicate key
 */
...
function _findObject(str, startInd, allowDuplicatedKeys, parse) {
  ...
  while (sepValue === ",") {
    ...
    if (allowDuplicatedKeys === "increment") {
      if (keys.indexOf(key.value) !== -1) {
        let count = duplicateKeys[key.value] || 1;
        duplicateKeys[key.value] = count + 1;
        key.value += count;
      }
    }
    keys.push(key.value);
